...
const DefaultLayout = ({ children, user, pageName }) => {
  ...
  useEffect(() => {
    const forceQuestionare = (!user.lastQuestionare || new Date().setHours(0, 0, 0, 0) - new Date(user.lastQuestionare).setHours(0, 0, 0, 0) >= 30 * 24 * 60 * 60 * 1000) && !window.location.pathname.includes("questionnaire");
        ...
        <div className="relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
          <main>
            <div className="mx-auto max-w-screen-2xl p-4 pl-[306px] w-full min-h-screen md:p-6 2xl:p-10 dark:bg-boxdark-2 dark:text-bodydark">
              ...
              <Link href="/questionnaire" className="inline-flex items-center justify-center rounded-lg bg-primary px-10 py-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 mt-10">
                Take the quiz
              </Link>
            </div>
          </main>
